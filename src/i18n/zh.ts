...
export const zh = {
  ...
  toolDescAsk:
    '出选择题。发给老板(to="boss")老板会看到可点选项并回选;发给同事则对方收到带编号选项的消息,用 sendmsg 回选哪个。注意:向 boss 提问会立即返回 pending(不阻塞、老板未必马上答)。提问后请**立刻调用 idle 结束本回合**等待——老板回答会作为新消息重新唤起你;切勿轮询、重复提问或空转重发(会被护栏拦下、徒耗回合)。',
  askPendingNote:
    '已转交 boss(等待其回答)。请现在就调用 idle 结束本回合——老板回答后会作为新消息重新唤起你。不要轮询、不要重复提问、不要空转重发。',
  undeliverableBlocked: (to: string) =>
    `检测到你在重复发送 / 空转(被循环护栏拦下),消息未送达:${to}。请立刻调用 idle 结束本回合等待——不要重发或重问;有新消息会自动重新唤起你。`,
  askBlockedInTodo:
